...
def load_model(args):
    ...
    if args.start_epo_idx >= 0:
        ...
        map_location = 'cuda:%d' % 0
        time_enc.load_state_dict(torch.load(f'{args.model_save_path}time_encoder_{args.start_epo_idx}_{args.mask_loss:.6f}.pt', map_location=map_location))
        ch_enc.load_state_dict(torch.load(f'{args.model_save_path}channel_encoder_{args.start_epo_idx}_{args.mask_loss:.6f}.pt', map_location=map_location))
...
if __name__ == '__main__':
    ...
    # parser.add_argument("--seq_len", type=int, default=15, help='this is the dim of L in the paper')
    parser.add_argument("--seq_len", type=int, default=150, help='this is the dim of L in the paper')
    ...
    parser.add_argument("--data_path", type=str, default='/home/jz/hard_disk/data/20231122_huanshan_sleep_756subs_patch/')
    parser.add_argument("--edf_data_path", type=str,
                        default='/home/jz/hard_disk/data/20231116_huashan_sleep_756subs/bipolar_EEG_data/')

    parser.add_argument("--use_power", type=str2bool, default=False)
    parser.add_argument("--mask_by_channel", type=str2bool, default=False)
    parser.add_argument("--rand_mask", type=str2bool, default=True)
    parser.add_argument("--mask_len", type=str2bool, default=None)
    parser.add_argument("--rec_down_samp_rate", type=int, default=1)

    parser.add_argument("--num_ch", type=int, default=10)

    parser.add_argument("--files_len", type=int, default=30, help='instance num of training dataset, -1 means the whole dataset')

    parser.add_argument("--is_normalize", type=str2bool, default=True, help='whether to normalize original signal')

    parser.add_argument("--device", type=int, default=0, help='whether to normalize original signal')
    parser.add_argument("--writer_folder", type=str, default='logs', help='where to save the tensorboard result')

    parser.add_argument("--dataloader_verbose", type=str2bool, default=False, help='whether to show the loading time and normalize time')

    parser.add_argument("--data_type", type=str, default='.edf', help='which data type used, choose from .edf or ...')
    parser.add_argument("--model_save_epoch", type=int, default=5, help='how many epoch for saving model')

    # 下面是关于加载模型的内容
    parser.add_argument("--model_save_path", type=str, default='/home/jz/hard_disk/data/20231129_brant_model_saving/', help='where to save model')
    parser.add_argument("--mask_loss", type=float, default=83.692177, help='the trained model mask loss')
    parser.add_argument("--start_epo_idx", type=int, default=-1,
                        help='load the model from last train, -1 means a new model')

    parser.add_argument("--compute_mean_std", type=str2bool, default=False,
                        help='get the mean and variance on the whole dataset, True for compute, False for load computed result')
