...
class InputEmbedding(nn.Module):
    def __init__(self, in_dim, c_dim, seq_len, d_model, band_num, project_mode, learnable_mask):
        ...
        # self.positional_encoding = nn.Parameter(torch.randn(seq_len, d_model, dtype=torch.float64), requires_grad=True)  # learnable positional encoding
        ...
        if learnable_mask:
            ...
            # self.mask_encoding = nn.Parameter(torch.zeros(in_dim, dtype=torch.float64), requires_grad=False)
            # self.power_mask_encoding = nn.Parameter(torch.zeros(d_model, dtype=torch.float64), requires_grad=False)
        ...
        if project_mode == 'cnn':
            ...
            self.proj = nn.Sequential(
                # nn.Linear(in_dim, d_model, dtype=torch.float64),
...
class TimeEncoder(nn.Module):
    ...
    def forward(self, mask, data, power=None, need_mask=True, mask_by_ch=False, rand_mask=True, mask_len=None, use_power=True):
